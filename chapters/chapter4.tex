...
\begin{itemize}
    ...
    \item \emph{Vision-based PI} (\texttt{detection\&pid.py}). The reference is reconstructed at every tick from the RGB camera by the CLRNet lane detector~\cite{clrnet} followed by the inverse-perspective-mapping projection of Section~\ref{sec:vision}. The same PI controller of the previous architecture consumes the heading error computed in the body frame. At intersections the \ac{LKA} hands off to the CARLA Traffic Manager autopilot, as described in Section~\ref{sec:vision}. The trajectory is logged to \texttt{ego\_trajectory\_vision.csv} and appears in red.
\end{itemize}
